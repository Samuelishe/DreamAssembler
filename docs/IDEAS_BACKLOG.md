...
Важно: текущий archive / transport / bureaucracy слой - это только первый strongest coherent atmospheric field проекта. Он не должен стать потолком identity.

Уже выращенные manifolds, которые нужно учитывать как существующую карту проекта, а не как wishlist:
...
- развивать DreamAssembler как atmospheric reading instrument, а не как control-heavy generator surface;
...
- выращивать несколько atmospheric semantic spaces, а не застывать в одном archive-field;
- позволять atmospheric fields мягко смешиваться, интерферировать и перетекать друг в друга.
- отделять neutral atmospheric foundation от старого urban/archive emotional baseline.

## Short-Term

Это ближайшие зрелые шаги, которые можно делать без ломки архитектуры.

- ввести `rhythm diversification pass` по templates: `announcement`, `inventory`, `procedural note`, `museum label`, `quiet instruction`, `interrupted memory`, `static observation`, `delayed revelation`, `emotionally suspended statement`;
- провести `foundation neutralization pass`: перестать считать `city / archive / bureaucracy / transport / paper / provincial` нейтральным baseline;
- перевести foundation ближе к `silence / waiting / maintenance / transition / repetition / residue / interval / dim_light / observation / drift / hum / routine`;
- ослаблять одинаковую длину дыхания фраз, одинаковый emotional pacing и слишком одинаковую sentence-shape;
- ввести manifold-specific cadence families, чтобы `weather`, `observatory`, `hydroelectric`, `mall`, `hospitality`, `museum` различались не только по nouns;
- усиливать very short fragments, almost-poetry structures и fragments built around state, observation, object resonance и procedural detail;
- усиливать `silence architecture`: больше templates почти без действия, почти без сюжета, вокруг residue, observation, static state и document-fragment feel;
- добавить very soft atmospheric continuity внутри batch: symbolic recurrence, motif resurfacing, emotional echoes, atmospheric resurfacing;
- разрешить hidden continuity между fragments без буквального повтора и без тяжелой memory-системы;
- ввести dominant emotional pressure как мягкий bias layer: `fluorescent`, `exhausted`, `anxious`, `ritualized`, `rainy`, `sterile`, `quiet`, `transit-heavy`, `archival`, `procedural`;
- расширить dominant pressure так, чтобы он удерживал не только nouns, но и pacing, implication, silence structure и observation style;
- развивать reading flow вместо isolated generation: remembered fragments, atmospheric trails, temporary session memory, sequence reading;
- усиливать lexical modes как отдельный atmospheric instrument: phrase-centered presentation, spotlight feel, mood continuity, batch coherence, isolated fragment resonance;
- добавить lightweight runtime surfacing audit без dashboard-логики: смотреть manifold surfacing, cadence dominance и hidden baseline drift;
- растить existing manifolds как emotional ecologies, а не как packs слов: rituals, service behavior, maintenance logic, institutional memory, recurring anxieties;
- отложить следующую большую expansion-wave до завершения runtime rebalance;
- продолжать curated-cleanup только в той мере, в которой он поддерживает rhythm, resonance и continuity, а не превращается в endless polish-loop.

## Medium-Term

Это следующий слой развития после стабилизации ближайших UI и data-улучшений.

- добавить atmosphere tags поверх текущих тегов: `rainy`, `archive`, `bureaucratic`, `provincial`, `night_shift`, `urban_decay`, `transport`, `anxious`, `dreamlike`, `transit`, `sterile_light`, `ritual`, `cosmic_quiet`, `hospitality`, `commerce_decay`, `gigantic`, `domestic_impossible`;
...
- развивать symbolic interference: когда мотивы одной atmospheric zone просачиваются в другую и создают новое поле, а не просто шум;
- развивать continuity между outputs не как сюжет, а как permeable atmospheric field;
- усиливать motif memory и resurfacing для batch и session-level reading;
- выращивать manifold-specific rhythm classes, forms of silence и procedural cadences;
- отделить atmospheric physics tags от world-imagery tags, чтобы foundation-layer стал эмоционально прозрачнее;
- проверять новые manifolds не только по dataset-mass, но и по наличию собственной runtime cadence-identity;
...
- motif resurfacing для объектов вроде ключей, номерков, жетонов, табличек, схем, витрин, аудиогидов и стекла;
- symbolic interference между curated fields: например, sterile transit + ritual bureaucracy или exhausted commerce + dreamlike logistics;
...
- isolated reading presentation, где короткий фрагмент живет как отдельный atmospheric object, а не как widget в списке;
- отдельные curated output modes наподобие `тихо`, `промышленно`, `дождливо`, `служебно`, `провинциально`, `ночная смена`;
- non-urban atmospheric fields через infrastructure-near-nature lens: observatories, reservoirs, ferries, radar stations, mountain transit, botanical maintenance;
...
- превращение visual polish в дизайнерский аттракцион;
...
- фиксация проекта в одном "постсоветском архивном" коридоре как будто это и есть вся его identity.
...
- попытка компенсировать слабую atmospheric curation "умными" системами вместо лучшего dataset design.
...
- permeability между пространствами, когда одна атмосфера мягко заражает другую.
...
Потенциальные следующие curated fields:
...
Особенно перспективные next-wave fields:
...
- fullscreen reading mode является одним из strongest направлений проекта;
...
- пустота, воздух и спокойная композиция являются частью experience;
- карточки должны ощущаться ближе к fragments / sheets / extracted memories, чем к обычным UI widgets.
