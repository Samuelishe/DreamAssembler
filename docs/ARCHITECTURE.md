...
- `TextGeneratorService` - главная точка генерации текста, учитывающая slot-требования, теги, compat-теги, анти-повторы, композиционные роли и базовый порядок narrative-фраз внутри `ShortText`.
...
5. Для `ShortText` сервис дополнительно следит за композиционными ролями шаблонов и их порядком, чтобы не собирать текст из одинаковых meta-ремарок и не путать временную композицию.
6. Для каждой категории из шаблона выбирается запись словаря с учетом веса, тегов, compat-тегов, слота и близости к выбранному уровню.
