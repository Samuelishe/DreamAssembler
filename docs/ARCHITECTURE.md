...
- `TextGeneratorService` - главная точка генерации текста, учитывающая slot-требования, теги и анти-повторы.
...
6. Расширить логику весов или тегов в `TextGeneratorService`, не меняя UI-слой.
7. Позже можно добавить редактор словарей как отдельный модуль, не меняя базовую структуру `Core`.
