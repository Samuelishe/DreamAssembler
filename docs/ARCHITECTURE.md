...
7. Если одна категория ведет себя по-разному в разных фразах, разводить ее по отдельным slot-подтипам вместо грубой общей формы, как это уже сделано для `emotion` и `twist`.
