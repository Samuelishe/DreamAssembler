...
- затем expansion-фокус возвращен к данным: добавлен новый curated manifold `dead shopping malls / abandoned commerce / fluorescent afterhours` по `place / object / condition / concept / atmosphere / twist`, а версия `data-manifest.json` поднята до `0.7.0`; `DreamAssembler.DataTools` показывает `379` entries и не находит structural issues;
...
- затем по этой новой статистике расширена airport-ось вторым curated pack: добавлены `character/airport_night_staff`, `action/airport_afterhours_actions`, `place/airport_afterhours_places`, `object/airport_afterhours_objects`, `condition/airport_afterhours_conditions`, `concept/airport_afterhours_concepts`, `atmosphere/airport_afterhours_tones`, `twist/airport_afterhours_twists`; версия `data-manifest.json` поднята до `0.8.0`, а общий corpus вырос до `436` entries без structural issues;
