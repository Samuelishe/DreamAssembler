...
- Введена категория `condition` для безопасных фразовых состояний.
- Словари уже переведены на древовидную структуру с `data-manifest.json` и полем `slot`.
- В решении есть `DreamAssembler.DataTools` для проверки качества набора данных.
- Пользовательские настройки сохраняются между запусками в локальный JSON-файл.
- В UI статусы разделены на данные, настройки и действия пользователя.
- WPF UI уже умеет генерировать, копировать и очищать историю.
- Для `ShortText` введены композиционные роли шаблонов, чтобы уменьшать каркасные повторы.
- Часть рискованных `emotion`-форм уже разведена по отдельным слотам для разных типов фраз.
- `twist` начал делиться на более безопасные slot-подтипы для общих и персонажных конструкций.
