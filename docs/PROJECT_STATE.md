...
3. Дальше усиливать atmospheric presentation только там, где оно реально меняет чтение, а не просто внешний вид.
4. Постепенно вводить atmospheric layers и atmosphere tags поверх текущей системы.
5. Дорабатывать narrative rhythm в `ShortText`.
6. Продолжать anti-generic filtering для словарей и шаблонов.
7. Оставить морфологические улучшения как поздний, опциональный слой.
