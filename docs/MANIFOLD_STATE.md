...
- runtime reports `audit-shorttext-e.json`, `audit-shorttext-f.json`, `audit-shorttext-long-f.json`, `audit-mall-g.json`, `audit-shorttext-long-g.json`, `audit-sentence-long-g.json`, `audit-hospitality-baseline.json`, `audit-hospitality-after.json`
...
| `radar_stations` | 8 | 48 | action, atmosphere, character, concept, condition, object, place, twist | compact first-wave | не делать runtime verdict заранее; следующий шаг только после обычного content cadence |
...
Комментарий:
Открыт новый compact manifold quiet signal infrastructure: rotating antennas, duty-room silence, azimuth sheets, delayed marks, horizon bureaucracy и procedural listening. Это пока first-wave field без отдельного long-runtime verdict.
...
`audit-hospitality-after.json` (`ShortText`, `1000`, `Absurd`) после hospitality autonomy pass показал:

- dominant pressure lead: `sanatorium:quiet` на `19.6%`, а не универсальный `quiet`;
- lead manifold surfacing: `hospitality` на `22.0%`;
- `hospitality` preferred cadence activation: `92.3%`;
- `mall` surfacing: `10.0%` with preferred cadence activation `93.0%`;
- `museum` surfacing: `6.0%` with preferred cadence activation `73.3%`;
- `observatory` surfacing: `4.0%` with preferred cadence activation `87.5%`;
- primary cadence lead: `static_observation` на `27.1%`, то есть он остается heavy skeleton, но не возвращается к прежнему `~40%`;
- `weather_systems` preferred cadence activation: `93.6%`;
- `hydroelectric` preferred cadence activation: `91.7%`;
- legacy-heavier outputs: `19.4%`.

Дополнительно `audit-sentence-long-g.json` (`Sentence`, `1000`, `Absurd`) показал:

- `museum` lead manifold surfacing: `22.0%`;
- `mall` surfacing: `10.0%`;
- `observatory` surfacing: `12.0%`;
- dominant pressure lead: `museum:quiet` на `21.3%`;
- legacy-heavier outputs: `17.4%`.

### Практический вывод

- проблема сейчас не в нехватке non-urban manifolds;
- `weather_systems`, `hydroelectric`, `observatory`, `mall` и теперь уже `hospitality` показывают сильную runtime cadence autonomy;
- contextual quiet differentiation сработала: universal `quiet` перестал быть единственным carrying pressure;
- `mall` после deepening уже не выглядит lexical-only: он стабильно surface-ится на длинной дистанции и удерживает сильную preferred cadence activation;
- `museum` больше не выглядит hidden universal baseline для всего runtime, но остается самым тяжелым manifold в `Sentence` и still under-activates cadence relative to `weather/hydroelectric/observatory/mall`;
- hospitality autonomy pass сработал: `hospitality` перестал быть near-invisible и поднялся с `2% / 45%` до `22% / 92.3%` в длинном `ShortText` срезе;
...
Сейчас это нужно оценивать не по одному `samples`-запуску, а по `report` и `compare`. На длинном `1000` срезе видно, что `mall`, `observatory` и `hydroelectric` реально живут в ecology, а не существуют только в corpus.
...
- `museum` в `Sentence` все еще тянет на себя lead role заметнее, чем остальные fields;
- `airport` теперь выглядит более слабым mature field, чем `hospitality`, особенно по cadence autonomy;
...
- новый `radar_stations` pack пока не должен оцениваться по отсутствию в нескольких коротких samples: это content-step, а не доказанный runtime problem.
...
3. `mall` отдельным emergency-pass пока не нужен: long audit уже показал, что он surface-ится и не является lexical-only;
4. hospitality autonomy pass можно считать успешным и закрытым; следующий runtime candidate сейчас скорее `museum overreach in Sentence` или `airport cadence autonomy`;
5. `museum` deepening делать только точечно и только если он помогает cadence identity, а не просто увеличивает corpus mass;
6. `radar_stations` не нуждается в немедленном runtime intervention; сначала дать проекту еще 1-2 content steps или обычный later checkpoint;
7. новые manifolds открывать только после очередного audit-backed runtime checkpoint.
