...
- runtime reports `audit-shorttext-e.json`, `audit-shorttext-f.json`, `audit-shorttext-long-f.json`, `audit-mall-g.json`, `audit-shorttext-long-g.json`, `audit-sentence-long-g.json`
...
`audit-shorttext-long-g.json` (`ShortText`, `1000`, `Absurd`) показал:

- dominant pressure lead: `sanatorium:quiet` на `23.9%`, а не универсальный `quiet`;
- lead manifold surfacing: `sanatorium` на `24.0%`;
- `mall` surfacing: `10.0%` with preferred cadence activation `94.0%`;
- `museum` surfacing: `8.0%` with preferred cadence activation `67.5%`;
- `observatory` surfacing: `8.0%` with preferred cadence activation `91.3%`;
- `hospitality` surfacing: `2.0%` with preferred cadence activation `45.0%`;
- primary cadence lead: `static_observation` на `27.7%`, то есть он уже заметно ниже прежнего `~40%`, но все еще остается самым тяжелым skeleton;
- `weather_systems` preferred cadence activation: `97.1%`;
- `hydroelectric` preferred cadence activation: `90.0%`;
- legacy-heavier outputs: `19.7%`.
...
- `hospitality` на длинном `ShortText` срезе стал самым слабым полем по cadence autonomy;
...
- `hospitality` сейчас выглядит самым слабым mature manifold по cadence autonomy в `ShortText`;
...
4. следующий runtime candidate сейчас скорее `hospitality cadence autonomy` или `museum overreach in Sentence`, чем `mall`;
